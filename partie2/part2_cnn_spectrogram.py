...
class CNNEncoder(nn.Module):
    ...
    def __init__(self, input_size, output_size, dropout=0.3):
        ...
        # CNN layers expect (batch, channels, time, features)
        # We'll add channel dimension in forward
        ...
        # After 2 maxpool layers: input_size // 4
    ...
    def forward(self, x):
        ...
        x = x.permute(0, 2, 1, 3)  # (batch, time, channels, features)
        x = x.reshape(batch_size, time, channels * features)
...
def train_epoch(model, dataloader, optimizer, criterion, device):
    ...
    with tqdm(dataloader, desc="Training") as pbar:
        for features, transcripts, feature_lengths, transcript_lengths in pbar:
            ...
            # Adjust feature lengths after convolution (2 maxpool layers)
            feature_lengths = feature_lengths // 2 // 2
...
def main():
    ...
    print(f"Spectrogram n_mels: {feature_extractor.n_mels}")
    ...
    # Initialize model
    input_size = feature_extractor.n_mels
    output_size = text_encoder.vocab_size
    
    model = CNNEncoder(input_size, output_size)
    ...
    plt.xlabel('Epoch')
    plt.ylabel('Loss')
    plt.title('Training Curves - CNN + Mel-Spectrogram + CTC')
    plt.legend()
    plt.grid(True)
    plt.savefig('part2_training_curves.png')
    print("\nTraining curves saved to 'part2_training_curves.png'")
    ...
    print("Model saved to 'part2_model.pth'")
